  ...
  render() {
    var displayNewsletterArticles = this.props.articles.map( (article, index) => {
      if(article.newsletter_id === this.props.newsletter.id) {
        ...
        return(
          <ArticleCard key={article.id} newsletter={this.props.newsletter} newsletterId={this.props.newsletter.id} newsletterName={this.props.newsletter.name} article={article} backgroundColor={backgroundColor}/>
        )
      }
      return '';
    })
